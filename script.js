...
import {convertPositionToIndex,SAD, PLAYFIELD_COLUMNS, PLAYFIELD_ROWS} from "./utilities.js";
...
function initTouch() {
    ...
    hammer.get('pan').set({deriction: Hammer.DIRECTION_ALL});
    hammer.get('swipe').set({deriction: Hammer.DIRECTION_ALL});
...
function draw() {
    cells.forEach(cells => cells.removeAttribute('class'))
    drawPlayField();
    drawTetromino();
    drawGhostTetromino();
}
...
function drawSad() {
    const TOP_OFFSET = 5;
    for (let row = 0; row < SAD[0].length; row++) {
